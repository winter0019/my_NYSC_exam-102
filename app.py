...
    if db:
        try:
            history_ref = db.collection('quiz_activities').where('user_email', '==', email).order_by('timestamp', direction=firestore.Query.DESCENDING).limit(10).stream()
...
                room_ref.update({'last_activity': firestore.SERVER_TIMESTAMP})
            else:
                messages = []
        else:
...
                room_data['last_activity'] = datetime.now()
            else:
...
    
    except Exception as e:
        logger.error(f"Error getting messages for room {room_id}: {e}", exc_info=True)
...
    
    except Exception as e:
        logger.error(f"Error sending message to room {room_id}: {e}", exc_info=True)
        return jsonify({"error": "An internal error occurred"}), 500
...
    try:
        if db:
            rooms_ref = db.collection('discussion_rooms').where('public', '==', True).order_by('last_activity', direction=firestore.Query.DESCENDING).stream()
            room_list = []
...
            active_rooms = sorted(
                [{"id": k, **v} for k, v in rooms.items() if v.get("public", False)],
                key=lambda x: x["last_activity"],
                reverse=True
            )
            return jsonify(active_rooms)
    except Exception as e:
        logger.error(f"Error listing discussion rooms: {e}", exc_info=True)
...
        
        # Log the quiz result
        log_quiz_activity(session["user_email"], quiz_type, subject, correct_count)
        
        return jsonify({"ok": True, "message": "Quiz result logged successfully."})
    
    except Exception as e:
        logger.error(f"Error submitting quiz: {e}", exc_info=True)
        return jsonify({"ok": False, "error": "Failed to log quiz result."}), 500

def fetch_user_history(email, limit=10):
    """Helper to fetch user history from Firestore"""
    if not db:
        return []
    try:
